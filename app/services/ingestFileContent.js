...
import { createTempFile, cleanupTempFile, createMetadata } from "./helpers.js";

export async function ingestPdfFile(file, vectorStore) {
  let filePath;
  try {
    ...
    const enrichedDocs = splitDocs.map((doc, idx) => ({
      ...
      metadata: {
        ...doc.metadata,
        ...createMetadata(file.name, "pdf", { chunkIndex: idx }),
      },
    }));
...
export async function ingestCsvFile(file, vectorStore) {
  let filePath;
  try {
    filePath = await createTempFile(file);
    ...
    const enrichedDocs = docs.map((doc, idx) => ({
      ...
      metadata: {
        ...doc.metadata,
        ...createMetadata(file.name, "csv", { rowIndex: idx }),
      },
    }));
...
export async function ingestFiles(pdfFiles, csvFiles, vectorStore) {
  const results = {
  ...
  for (const file of pdfFiles) {
    const result = await ingestPdfFile(file, vectorStore);
    if (result.success) {
  ...
  for (const file of csvFiles) {
    const result = await ingestCsvFile(file, vectorStore);
    if (result.success) {
